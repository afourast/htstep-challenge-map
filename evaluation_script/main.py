# Modified from Actionformer evaluation code
# see https://github.com/happyharrycn/actionformer_release/blob/main/libs/utils/metrics.py
...
def load_gt_seg_from_json(json_db, split=None, label='label_id', label_offset=0):
    # load json file
    ...
    for k, v in json_db.items():
        ...
        ants = remove_duplicate_annotations(v['annotations'] if 'annotations' in v else v['annotation'])
        # video id
        vids += [k] * len(ants)
...
class ANETdetectionGrouped(object):
    ...
    def __init__(
        ...
        assert label_groups is not None, 'Need to supply label groups'
        ...
        self.ground_truth = load_gt_seg_from_json(
            ants, split=self.split, label=label, label_offset=label_offset)
        ...
        # -- map the labels to groups 
        self.label_groups = label_groups
        self.all_og_steps_in_gt = self.ground_truth['label'].unique()
        self.group_index = {j: i for i, j in enumerate(sorted(set([ self.label_groups[lab_id] for lab_id in self.all_og_steps_in_gt]))) }
        self.lab2group_idx = { kk: self.group_index[self.label_groups[kk]] for kk in self.all_og_steps_in_gt}
        assert self.ground_truth['label'].isin(self.lab2group_idx).all()

        self.ground_truth['group'] = self.ground_truth['label'].replace(self.lab2group_idx)
    ...
    def wrapper_compute_average_precision(self, preds):
        ...
        ap_group = np.zeros((len(self.tiou_thresholds), len(set(self.lab2group_idx.values()))))

        ground_truth_by_group = self.ground_truth.groupby('group')
        prediction_by_group = preds.groupby('group')

        results = Parallel(n_jobs=self.num_workers)(
            delayed(compute_average_precision_detection)(
                ground_truth=ground_truth_by_group.get_group(cidx).reset_index(drop=True),
                prediction=self._get_predictions_with_label(prediction_by_group, label_name, cidx),
                tiou_thresholds=self.tiou_thresholds,
            ) for label_name, cidx in self.group_index.items())

        for i, cidx in enumerate(self.group_index.values()):
            ap_group[:,cidx] = results[i]
    ...
    def evaluate(self, preds, verbose=True):
        ...
        if isinstance(preds, pd.DataFrame):
            ...
            preds = load_pred_seg_from_json(preds, label=self.label)
        ...
        preds = preds[preds['label'].isin(self.activity_index)].copy()

        preds['group'] = preds['label'].replace(self.lab2group_idx)
        preds['label'] = preds['label'].replace(self.activity_index)

        # compute mAP
        self.ap_group = self.wrapper_compute_average_precision(preds)
        # mAP = self.ap.mean(axis=1)
        # average_mAP = mAP.mean()

        mAP_group = self.ap_group.mean(axis=1)
        average_mAP_group = mAP_group.mean()
        ...
        if verbose:
            ...
            block = ''
            for tiou, tiou_mAP in zip(self.tiou_thresholds, mAP_group):
                block += '\n|tIoU = {:.2f}: mAP = {:.2f} (%)'.format(tiou, tiou_mAP*100)
            print(block)
            # print(f'Avearge mAP: {average_mAP*100:.2f} (%), Average mAP_grouped {average_mAP_group*100:.2f} (%)')
            print(f'Average mAP_grouped {average_mAP_group*100:.2f} (%)')

        # return the results
        return mAP_group, average_mAP_group
...
def evaluate_article_grounding(annotations_json, results_csv, split, tiou_thresholds):
    ...
    activity2step_ids = { kk: vv.global_step_index.values for kk,vv in  taxonomy.groupby('activity')  }
    activity2idx = { kk: ii for ii, (kk,vv) in enumerate(sorted(activity2step_ids.items(), key=lambda x:x[1][0]))}
    step_idx2activity_idx = {stp: activity2idx[act] for act, vv in activity2step_ids.items() for stp in vv}
    evaluator_group = ANETdetectionGrouped(
        ants,
        split,
        tiou_thresholds,
        label_groups=step_idx2activity_idx,
        label='id',
    )
